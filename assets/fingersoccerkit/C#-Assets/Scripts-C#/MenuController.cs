...
public class MenuController : MonoBehaviour
{
    ...
    private void language()
    {
        if (pubR.language_option == 0)
            ...
            {
                if (eng_text[i] != null) eng_text[i].SetActive(true);
                if (arab_text[i] != null) arab_text[i].SetActive(false);
            }
        else
            for (var i = 0; i < eng_text.Length; i++)
            {
                if (arab_text[i] != null) arab_text[i].SetActive(true);
                if (eng_text[i] != null) eng_text[i].SetActive(false);
            }
    }
