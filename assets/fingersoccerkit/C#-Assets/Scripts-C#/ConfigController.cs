...
public class ConfigController : MonoBehaviour
{
    ...
    private int p1FormationCounter; //Actual player-1 formation index
    private int p1TeamCounter = 11; //Actual player-1 team index
    private int p2FormationCounter; //Actual player-2 formation index
    private int p2TeamCounter = 11; //Actual player-2 team index
    private int timeCounter; //Actual game-time index

    private readonly bool[] isAvailableTeam = new bool[21];
    private readonly bool[] isAvailableForm = new bool[5];
    ...
    private void Awake()
    {
        ...
        p1FormationLabel.GetComponent<TextMesh>().text =
            availableFormations[p1FormationCounter]; //loads default formation
        ...
        p2FormationLabel.GetComponent<TextMesh>().text =
            availableFormations[p2FormationCounter]; //loads default formation
    ...
    private void checkAvailableTeam()
    {
        for (var i = 0; i < 21; i++) isAvailableTeam[i] = true;
    ...
    private void checkAvailableForm()
    {
        for (var i = 0; i < 5; i++) isAvailableForm[i] = true;
        ...
        if (Physics.Raycast(ray, out hitInfo))
        {
            ...
            switch (objectHit.name)
            {
                case "p1-TBR":
                    ...
                    p1Team.GetComponent<Renderer>().material.mainTexture =
                        availableTeams[p1TeamCounter]; //set the flag on UI
                    p1PowerBar.transform.localScale = new Vector3(TeamsManager.getTeamSettings(p1TeamCounter).x / 10,
                        p1PowerBar.transform.localScale.y,
                        p1PowerBar.transform.localScale.z);
                ...
                case "p2-TBR":
                    ...
                    p2Team.GetComponent<Renderer>().material.mainTexture =
                        availableTeams[p2TeamCounter]; //set the flag on UI
                    p2PowerBar.transform.localScale = new Vector3(TeamsManager.getTeamSettings(p2TeamCounter).x / 10,
                        p2PowerBar.transform.localScale.y,
                        p2PowerBar.transform.localScale.z);
                ...
                case "p1-FBL":
                    ...
                    p1FormationLabel.GetComponent<TextMesh>().text =
                        availableFormations[p1FormationCounter]; //set the string on the UI
                    break;
    ...
    //*****************************************************************************
    private void fixCounterLengths()
    {
